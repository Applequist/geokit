...
impl DefaultTransformationProvider {
    ...
    pub fn convert(&self, src: &Crs, dst: &Crs) -> TransformationPath {
        debug_assert_eq!(src.datum(), dst.datum());
        match src {
            Crs::Geocentric { datum, axes: src_axes, .. } => self.convert_from_geocentric(datum, src_axes, dst),
            Crs::Geographic { datum, axes: src_axes, .. } => self.convert_from_geographic(datum, src_axes, dst),
            Crs::Projected { datum, axes: src_axes, projection: src_projection, .. } => self.convert_from_projected(datum, src_axes, src_projection, dst),
        }
    }

    fn convert_from_geocentric(&self, datum: &GeodeticDatum, _src_axes: &GeocentricAxes, dst: &Crs) -> TransformationPath {
        match dst {
            ...
            Crs::Projected { axes: dst_axes, projection: dst_projection, .. } => {
    ...
    fn convert_from_geographic(&self, datum: &GeodeticDatum, src_axes: &GeodeticAxes, dst: &Crs) -> TransformationPath {
        match dst {
            Crs::Geocentric { .. } => {
    ...
    fn convert_from_projected(&self, datum: &GeodeticDatum, src_axes: &ProjectedAxes, src_projection: &ProjectionSpec, dst: &Crs) -> TransformationPath {
        match dst {
            Crs::Geocentric { .. } => {
                let src_to_dst = Normalization::from(*src_axes)
...
impl TransformationProvider for DefaultTransformationProvider {
    fn transformation(
        ...
        if src.datum() == dst.datum() {
            return Ok(self.convert(src, dst));
        }
